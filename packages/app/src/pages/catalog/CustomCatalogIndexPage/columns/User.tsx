import { CompoundEntityRef, RELATION_MEMBER_OF, stringifyEntityRef } from "@backstage/catalog-model";
import { Avatar, Link, TableColumn } from "@backstage/core-components";
import { CatalogTableColumnsFunc, CatalogTableRow } from "@backstage/plugin-catalog";
import { EntityRefLink, getEntityRelations } from "@backstage/plugin-catalog-react";
import { Box, Chip } from "@material-ui/core";
import React from "react";
import { categorizeTags } from "./utils";
...
const createIdentityColumn: () => TableColumn<CatalogTableRow> = () => ({
    ...
    width: 'auto',
    render: ({ entity }) => {
        const profile = entity.spec?.profile as { email?: string, displayName?: string, preferredUsername?: string };
...
const createSkillsColumn: () => TableColumn<CatalogTableRow> = () => ({
    ...
    width: 'auto',
    render: ({ entity }) => {
        const metadataTags = categorizeTags(entity?.metadata?.tags ?? []);

        if (!metadataTags.has('skill')) {
            return null;
        }

        return (
            <Box display="flex" flexDirection="column" gridRowGap="1rem">
                {metadataTags.get('skill')?.map(tag => (
...
const createTeamsColumn: () => TableColumn<CatalogTableRow> = () => ({
    ...
    width: 'auto',
    render: ({ entity }) => {
        const teams: CompoundEntityRef[] = getEntityRelations(entity, RELATION_MEMBER_OF);
